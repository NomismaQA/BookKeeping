...
public class Rec_Bank_AMTverify extends BasePage
{
	...
	public Rec_Bank_AMTverify (WebDriver driver)
	{
		super(driver);
	}
   ...
   public void AssertUrl()
	...
	public void Click_transactionno_click()
	{
		...
		elem.click();
		   utilities.PassScreenshot.Getscreenshot11("transactionn src","bank search page");
...
//	}
		...
		int t =m_Driver.findElements(By.xpath("//*[@id='aspnetForm']/div[3]/div[1]/div/table/tbody[2]/tr/td[3]")).size();
	  
		System.out.println("no of columns :" +t);
		for(int i=0;i<t-1;i++)
		{
			Credit=m_Driver.findElement(By.xpath("//*[@id='aspnetForm']/div[3]/div[1]/div/table/tbody[2]/tr["+(i+1)+"]/td[3]")).getText().replaceFirst("£","");
		  fy=Credit.replace(",","");
		System.out.println("Credit Amount i+1 =" +fy);
		dbamout=Float.parseFloat(fy);
		sum=sum+dbamout;
		sum=(float) (Math.round(sum *100)/100.0);
		GetScreenShot.captureAsImage(m_Driver,Credit);
		}
	...
	public void Click_Debit_Amount() throws IOException
	{
		...
		dbamout3=sum;
		Debit=m_Driver.findElement(By.xpath("//*[@id='aspnetForm']/div[3]/div[1]/div/table/tbody[2]/tr[3]/td[2]")).getText().replaceFirst("£","");
		g=Debit.replace(",","");
